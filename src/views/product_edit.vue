<template>
    ...
    <table class="table table-bordered table-striped">
      ...
      <tbody>
        <tr v-for="product in products" :key="product.product_id">
          ...
          <td>{{ product.stock }}</td>
          <td>
            <img :src="'http://localhost:8081/67709949/api_php/uploads/' + product.image"
            width="200" 
            height="150"
            :alt="product_name">
          </td>
          <td>
            <button class="btn btn-warning btn-sm" @click="openEditModal(product)"><i class="fa-solid fa-user-pen"></i></button> |      
            <button class="btn btn-danger btn-sm" @click="deleteCustomer(product.product_id)"><i class="fa-solid fa-trash"></i></button>
  ...
  <div v-if="isEditMode && editForm.image">
    ...
    <img :src="'http://localhost:8081/67709949/api_php/uploads/' + editForm.image"
            width="200" 
            height="150"
            :alt="product_name">
  </div>
...
export default {
  ...
  setup() {
    ...
    const fetchProducts = async () => {
      try {
        const res = await fetch("http://localhost:8081/67709949/api_php/api_product.php");
